...
local function setDataToDataStore(sectionName, user, data, deadline)
	...
	if not sectionSuccess then
		...
		warn("F - getDataFromDataStore - Failed set data section " .. user.userId .. ": " .. sectionName)
	end
...
local function awardBadge(user, badge)
	...
	-- If no badge with the name is found
...
local function loadInUser(user)
	...
	-- Update effect
	if savingNonBoolStatsFolder.effect.Value ~= 0 then
		if savingStatsFolder["effect"..savingNonBoolStatsFolder.effect.Value].Value == false then
			savingNonBoolStatsFolder.effect.Value = 0
		end
	end

	-- Update music
	if savingNonBoolStatsFolder.music.Value ~= 0 and savingNonBoolStatsFolder.music.Value ~= -1 then
		if savingStatsFolder["music"..savingNonBoolStatsFolder.music.Value].Value == false then
...
UpdateBackPackEvent.OnServerEvent:Connect(function(user, number)
	...
	-- If backpack exists
	if user.savingstats:FindFirstChild("back"..number) then
